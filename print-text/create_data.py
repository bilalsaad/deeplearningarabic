...
FONTS = ["Arial Unicode.ttf",
         ...
         "DecoTypeNash.tff",
         "Nadeem.ttc"]
DEFAULT_IMAGE_SIZES = [(100, 100)]
DEFAULT_FONT_SIZES = [30, 40, 50]
DEFAULT_LOCATIONS = [(25, 25)] # this should be probably be dependant on sizes 
...
def create_data(word, label, sizes = DEFAULT_IMAGE_SIZES,
    ...
    for (size, font_size, location) in itertools.product(
        ...
        print i
        process_single_image(word, label, size, font_size, location)
        i += 1
